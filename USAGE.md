...
> 터미널 대신 마우스로 클릭하는 화면을 원한다면, 이 문서 대신 아래 명령으로 브라우저
> 제어판을 켤 수도 있습니다:
> ```bash
> pip install -r requirements-web.txt
> python -m trading_agent.webapp
> ```
> 그 다음 브라우저에서 `http://127.0.0.1:8000` 접속. 종목·모드를 화면에서 고르고
> "세션 만들기" → "시작"만 누르면 됩니다 (일시정지·중단·결과 내보내기 버튼 포함). 자세한
> 구조는 README.md의 "Web UI" 항목 참고. 아래 1번부터는 터미널(CLI) 기준 안내입니다.
