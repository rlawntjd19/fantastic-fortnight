"""One-off: compare SNDK against other candidates in the same memory/AI-
hardware theme -- Micron (MU) and The Metals Company (TMC) -- using the
real committee analyst pipeline (same assess_symbol code path used
throughout this session), plus a real correlation matrix against the
existing basket. SKHY is not re-checked here -- already ruled out on a
real FX-parity check (46.5% premium to fair value, not a reliable
instrument). Not a permanent part of the pipeline; prints to the job
log, then this script and its workflow get deleted.
"""
...
CORR_BASKET = ["QQQM", "NBIS", "CRWV", "SNDK", "MU", "TMC"]

print("=" * 70)
print("Real 1-year correlation matrix (existing basket + MU, TMC)")
print("=" * 70)
frames = {}
for symbol in CORR_BASKET:
...
print()
print("=" * 70)
print("Eligibility + real committee assessment: MU, TMC")
print("=" * 70)
...
for symbol in ["MU", "TMC"]:
    ...
    print(f"  Name: {long_name} | Sector: {sector} | Industry: {industry} | Exchange: {exchange}")
